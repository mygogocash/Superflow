...
const TEMPLATE_SAMPLE_ROWS = [
  [
    ...
    "2026-01-14",
    "active",
    "BOI work permit issued in Bangkok",
  ],
];

interface VisaBulkImportDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImported?: () => void;
}

export function VisaBulkImportDialog({
  open,
  onOpenChange,
  onImported,
}: VisaBulkImportDialogProps) {
  const [file, setFile] = useState<File | null>(null);
  const [parsing, setParsing] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [preview, setPreview] = useState<VisaImportPreview | null>(null);
  ...
  function downloadTemplate(format: "xlsx" | "csv") {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Visa Records");
    XLSX.writeFile(
      wb,
      format === "xlsx"
        ? "visa-import-template.xlsx"
        : "visa-import-template.csv",
      format === "csv" ? { bookType: "csv" } : undefined,
    );
  }

  // Read file → xlsx workbook → first sheet → array of objects keyed by
  // header row. Both .xlsx and .csv go through the same code path —
  // SheetJS handles CSV natively.
  ...
  async function handlePreview() {
    ...
    try {
      ...
      const res = await previewVisaImport(rows);
      setPreview(res.data);
  ...
  async function handleCommit() {
    ...
    try {
      setSubmitting(true);
      const rows = await parseFile(file);
      const res = await commitVisaImport(rows);
      setCommitted({ imported: res.data.imported });
      toast.success(
        `Imported ${res.data.imported} ${
          res.data.imported === 1 ? "visa record" : "visa records"
        }`,
      );
      onImported?.();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle>Bulk import visas</DialogTitle>
          <DialogDescription>
            Upload an XLSX or CSV file with visa records. Preview validates
            rows; commit writes them to the database.
          </DialogDescription>
        </DialogHeader>
        ...
        <section className="flex flex-col gap-3">
          ...
          <p className="text-muted-foreground text-xs">
            Required: <span className="font-mono">visaType</span>,{" "}
            <span className="font-mono">expiryDate</span> (YYYY-MM-DD), and at
            ...
            renewed), <span className="font-mono">notes</span>.
          </p>
        ...
        {committed ? (
          <p className="text-xs text-emerald-600">
            ✅ Imported {committed.imported} visa record
